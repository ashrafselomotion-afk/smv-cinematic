...
HC=$(h "$T/site.min.css"); HJ=$(h "$T/site.min.js"); HT=$(h "$T/theme.min.js"); HM=00000000
rm -f assets/site.*.min.css assets/site.*.min.js assets/theme.*.min.js assets/space.*.min.js
cp "$T/site.min.css" "assets/site.$HC.min.css"; cp "$T/site.min.js" "assets/site.$HJ.min.js"; cp "$T/theme.min.js" "assets/theme.$HT.min.js"; 
...
for f in "$T/site.min.css" "$T/site.min.js"; do printf "%s raw=%s gz=%s\n" "$(basename $f)" "$(wc -c <"$f")" "$(gzip -c "$f" | wc -c)"; done
rm -rf "$T"
